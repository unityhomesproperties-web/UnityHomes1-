    ...
    <div className="min-h-screen flex flex-col bg-white overflow-hidden">
      ...
      <AnimatePresence>
        {activeComingSoon && (
          ...
          >
            <div className="absolute inset-0 bg-[#132A1D]/40 backdrop-blur-sm" onClick={() => setActiveComingSoon(null)}></div>
            <motion.div 
              initial={{ y: 20, scale: 0.95 }}
              animate={{ y: 0, scale: 1 }}
              exit={{ y: 20, scale: 0.95 }}
              transition={{ duration: 0.2 }}
              className="bg-white w-full max-w-lg rounded-[32px] p-8 md:p-12 shadow-2xl relative z-10 flex flex-col items-center text-center"
            >
              <button 
      ...
      <section className="relative py-24 sm:py-32 px-4 sm:px-6 lg:px-8 text-center bg-[#2F8D46] overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto space-y-8 relative z-10">
          ...
          >
            ...
            <Link
              to="/waitlist"
              className="inline-flex bg-white text-[#2F8D46] px-10 py-5 rounded-[18px] font-bold text-lg hover:-translate-y-0.5 active:translate-y-0 shadow-lg hover:shadow-xl transition-all duration-200"
            >
              Join The Waitlist
            </Link>
          </motion.div>
...
console.log('ServicesPage written');
