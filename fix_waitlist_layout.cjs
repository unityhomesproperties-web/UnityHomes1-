...
// We will replace the entire return block, and dynamically insert the switch block of the original forms.
...
// We need to extract the switch block that renders the forms.
// It's inside: <div className="mt-8 relative min-h-[400px]">
// Which contains: <AnimatePresence mode="wait"> ... </AnimatePresence>
const formSwitchStart = content.indexOf('<div className="mt-8 relative min-h-[400px]">');
// We need to find the matching closing div for this block.
let open = 0;
let formSwitchEnd = -1;
...
// For the Role Selection block, it is inside {currentStep === 1 && ( ... )} inside the AnimatePresence.
// Actually, we will just completely replace the Role Selection block within formSwitchBlock!
let formSwitchModified = formSwitchBlock.replace(
    /\{currentStep === 1 && \([\s\S]*?<\/div>\s*<\/motion\.div>\s*\)\}/,
    `{currentStep === 1 && (
                <motion.div
                  key="role-selection"
...
const finalRender = newRender.replace('FORM_CONTENT_PLACEHOLDER', formSwitchModified);
