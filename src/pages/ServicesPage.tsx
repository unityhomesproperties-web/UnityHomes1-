    ...
    <div className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto min-h-screen animate-fade-in relative">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-24">
        {SERVICES.map((service) => (
          <div key={service.id} className="bg-[var(--color-white)] p-8 rounded-[var(--radius-card)] border border-[var(--color-border)] shadow-sm flex flex-col h-full hover:shadow-md transition-shadow relative overflow-hidden">
            <div className="w-14 h-14 bg-[var(--color-background)] rounded-full flex items-center justify-center text-[var(--color-secondary-green)] mb-6">
              <service.icon className="w-7 h-7" />
            </div>
            <h3 className="text-xl font-bold text-[var(--color-primary-text)] mb-3">{service.title}</h3>
            <p className="text-[var(--color-secondary-text)] mb-8 flex-1 leading-relaxed">
              {service.description}
            </p>
            ...
            <div className={`absolute inset-0 bg-[var(--color-primary-green)] text-[var(--color-white)] p-8 flex flex-col justify-center items-center text-center transition-transform duration-300 ${activeComingSoon === service.id ? 'translate-y-0' : 'translate-y-full'}`}>
              ...
              <button
                ...
                className="text-[var(--color-accent-gold)] font-semibold hover:underline"
              >
                Close Panel
              </button>
            </div>
      ...
      <div className="space-y-16 max-w-4xl mx-auto border-t border-[var(--color-border)] pt-16">
        <section id="verification-details" className="scroll-mt-32">
          <h2 className="text-3xl font-bold text-[var(--color-primary-green)] mb-6">Property Verification details</h2>
          <p className="text-[var(--color-secondary-text)] text-lg leading-relaxed mb-4">
        ...
        <section id="management-details" className="scroll-mt-32">
          <h2 className="text-3xl font-bold text-[var(--color-primary-green)] mb-6">Property Management workflows</h2>
          <p className="text-[var(--color-secondary-text)] text-lg leading-relaxed mb-4">
        ...
        <section id="facilities-details" className="scroll-mt-32">
          <h2 className="text-3xl font-bold text-[var(--color-primary-green)] mb-6">Facilities Management integrations</h2>
          <p className="text-[var(--color-secondary-text)] text-lg leading-relaxed mb-4">
