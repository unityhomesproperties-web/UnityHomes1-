  ...
  return (
    <div className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto min-h-screen animate-fade-in">
      <div className="max-w-4xl mx-auto text-center mb-20">
        ...
        <p className="text-2xl text-[var(--color-text-primary)] font-medium leading-relaxed">
      ...
      <div className="relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-6 relative z-10">
          {SEQUENCE.map((item, idx) => (
            <div key={idx} className="bg-white p-6 rounded-[var(--radius-card)] border border-[var(--color-border)] shadow-sm flex flex-col items-center text-center relative hover:-translate-y-1 transition-transform">
              <div className="absolute -top-3 bg-[var(--color-surface-light)] px-2 py-1 text-xs font-bold text-[var(--color-text-secondary)] rounded-[var(--radius-pill)] border border-[var(--color-border)] shadow-sm">
                Step 0{idx + 1}
              </div>
              <h3 className="text-lg font-bold text-[var(--color-brand-deep)] mt-4 mb-2">{item.step}</h3>
              ...
              <div className={`px-3 py-1 text-xs font-semibold rounded-[var(--radius-pill)] ${
                item.status === 'Available Now' 
                  ...
                  : 'bg-[var(--color-surface-light)] text-[var(--color-text-secondary)] border border-[var(--color-border)]'
              }`}>
                {item.status}
              </div>
            </div>
