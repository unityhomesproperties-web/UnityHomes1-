  ...
  return (
    <div className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto min-h-screen flex flex-col justify-center animate-fade-in">
      ...
      <div className="grid md:grid-cols-3 gap-8 mb-16">
        {CARDS.map((card, idx) => (
          <div key={idx} className="bg-[var(--color-white)] p-8 rounded-[var(--radius-card)] border border-[var(--color-border)] text-center shadow-sm">
            <div className="w-16 h-16 bg-[var(--color-background)] rounded-full flex items-center justify-center text-[var(--color-secondary-green)] mx-auto mb-6">
              <card.icon className="w-8 h-8" />
            </div>
            <h3 className="text-xl font-bold text-[var(--color-primary-text)] mb-4">{card.title}</h3>
