...
const StarRating = ({ value, onChange, label }: { value: number, onChange: (val: number) => void, label: string }) => {
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-[var(--color-border)] rounded-[var(--radius-button)] bg-white hover:border-[var(--color-brand-fresh)] transition-colors">
      <span className="font-semibold text-[var(--color-text-primary)] mb-2 sm:mb-0 capitalize">{label.replace(/_/g, ' ')}</span>
          ...
          >
            <Star
              className={`w-8 h-8 sm:w-6 sm:h-6 transition-colors ${
                star <= (hover || value) ? 'fill-[var(--color-brand-medium)] text-[var(--color-brand-medium)]' : 'text-gray-300'
              }`}
            />
          </button>
        ))}
  ...
  return (
    <div className="py-12 md:py-24 px-4 sm:px-6 lg:px-8 max-w-3xl mx-auto min-h-screen">
      <div className="mb-10 text-center animate-fade-in">
        <h1 className="text-3xl font-bold text-[var(--color-brand-deep)] mb-4">Help Build Better Area Intelligence</h1>
        <p className="text-[var(--color-text-secondary)]">Share real information about your neighbourhood to help people make better property decisions.</p>
        <p className="text-sm font-semibold text-[var(--color-brand-medium)] mt-2">Estimated time: 3 to 4 minutes</p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {currentStep === 1 && (
            <div className="space-y-6 animate-fade-in">
              <h2 className="text-xl font-bold text-[var(--color-text-primary)] mb-6 pb-4 border-b border-[var(--color-border)]">Location Details</h2>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-semibold text-[var(--color-text-primary)] mb-2">State</label>
                  <input type="text" value={data.location.state} onChange={e => updateSection('location', 'state', e.target.value)} onBlur={() => handleBlur('state')} className="w-full px-4 py-4 rounded-[var(--radius-input)] border border-[var(--color-border)] bg-[var(--color-surface-light)] focus:outline-none focus:border-[var(--color-brand-fresh)] text-[var(--color-text-primary)]" placeholder="e.g. Lagos" />
                ...
                <div>
                  ...
                  <input type="text" value={data.location.lga} onChange={e => updateSection('location', 'lga', e.target.value)} onBlur={() => handleBlur('lga')} className="w-full px-4 py-4 rounded-[var(--radius-input)] border border-[var(--color-border)] bg-[var(--color-surface-light)] focus:outline-none focus:border-[var(--color-brand-fresh)] text-[var(--color-text-primary)]" placeholder="e.g. Eti-Osa" />
              ...
              <div>
                ...
                <input type="text" value={data.location.area} onChange={e => updateSection('location', 'area', e.target.value)} onBlur={() => handleBlur('area')} className="w-full px-4 py-4 rounded-[var(--radius-input)] border border-[var(--color-border)] bg-[var(--color-surface-light)] focus:outline-none focus:border-[var(--color-brand-fresh)] text-[var(--color-text-primary)]" placeholder="e.g. Lekki Phase 1" />
                {renderError('area', errors)}
              </div>

              <div>
                <label className="block text-sm font-semibold text-[var(--color-text-primary)] mb-2">Estate Name (Optional)</label>
                <input type="text" value={data.location.estate} onChange={e => updateSection('location', 'estate', e.target.value)} className="w-full px-4 py-4 rounded-[var(--radius-input)] border border-[var(--color-border)] bg-[var(--color-surface-light)] focus:outline-none focus:border-[var(--color-brand-fresh)] text-[var(--color-text-primary)]" placeholder="e.g. 1004 Estate" />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-semibold text-[var(--color-text-primary)] mb-2">Years Lived/Known Here</label>
                  <input type="number" min="0" value={data.location.years_lived} onChange={e => updateSection('location', 'years_lived', e.target.value)} onBlur={() => handleBlur('years_lived')} className="w-full px-4 py-4 rounded-[var(--radius-input)] border border-[var(--color-border)] bg-[var(--color-surface-light)] focus:outline-none focus:border-[var(--color-brand-fresh)] text-[var(--color-text-primary)]" placeholder="e.g. 3" />
                  {renderError('years_lived', errors)}
                </div>
                <div>
                  <label className="block text-sm font-semibold text-[var(--color-text-primary)] mb-2">Relationship to Area</label>
                  <select value={data.location.relationship} onChange={e => updateSection('location', 'relationship', e.target.value)} onBlur={() => handleBlur('relationship')} className="w-full px-4 py-4 rounded-[var(--radius-input)] border border-[var(--color-border)] bg-[var(--color-surface-light)] focus:outline-none focus:border-[var(--color-brand-fresh)] text-[var(--color-text-primary)]">
          ...
          {currentStep === 2 && (
            <div className="space-y-6 animate-fade-in">
              <h2 className="text-xl font-bold text-[var(--color-text-primary)] mb-2 pb-4 border-b border-[var(--color-border)]">Area Experience</h2>
              <p className="text-sm text-[var(--color-text-secondary)] mb-6">Rate the following aspects from 1 (Very Poor) to 5 (Excellent). Leave blank if you don't know.</p>
          ...
          {currentStep === 3 && (
            <div className="space-y-6 animate-fade-in">
              <h2 className="text-xl font-bold text-[var(--color-text-primary)] mb-6 pb-4 border-b border-[var(--color-border)]">Housing Information</h2>
              
              <div>
                <label className="block text-sm font-semibold text-[var(--color-text-primary)] mb-2">Typical Rent for a 2-Bedroom (Estimated)</label>
                <input type="text" value={data.housing.typical_rent} onChange={e => updateSection('housing', 'typical_rent', e.target.value)} onBlur={() => handleBlur('typical_rent')} className="w-full px-4 py-4 rounded-[var(--radius-input)] border border-[var(--color-border)] bg-[var(--color-surface-light)] focus:outline-none focus:border-[var(--color-brand-fresh)] text-[var(--color-text-primary)]" placeholder="e.g. ₦3,000,000" />
              ...
              <div>
                <label className="block text-sm font-semibold text-[var(--color-text-primary)] mb-2">Housing Availability</label>
                <select value={data.housing.availability} onChange={e => updateSection('housing', 'availability', e.target.value)} onBlur={() => handleBlur('availability')} className="w-full px-4 py-4 rounded-[var(--radius-input)] border border-[var(--color-border)] bg-[var(--color-surface-light)] focus:outline-none focus:border-[var(--color-brand-fresh)] text-[var(--color-text-primary)]">
              ...
              <div>
                <label className="block text-sm font-semibold text-[var(--color-text-primary)] mb-2">Ease of Finding Accommodation</label>
                <select value={data.housing.ease_of_finding} onChange={e => updateSection('housing', 'ease_of_finding', e.target.value)} onBlur={() => handleBlur('ease_of_finding')} className="w-full px-4 py-4 rounded-[var(--radius-input)] border border-[var(--color-border)] bg-[var(--color-surface-light)] focus:outline-none focus:border-[var(--color-brand-fresh)] text-[var(--color-text-primary)]">
              ...
              <div>
                <label className="block text-sm font-semibold text-[var(--color-text-primary)] mb-2">Rent Change Experience (Past 2 Years)</label>
                <select value={data.housing.rent_change} onChange={e => updateSection('housing', 'rent_change', e.target.value)} onBlur={() => handleBlur('rent_change')} className="w-full px-4 py-4 rounded-[var(--radius-input)] border border-[var(--color-border)] bg-[var(--color-surface-light)] focus:outline-none focus:border-[var(--color-brand-fresh)] text-[var(--color-text-primary)]">
          ...
          {currentStep === 4 && (
            <div className="space-y-6 animate-fade-in">
              <h2 className="text-xl font-bold text-[var(--color-text-primary)] mb-6 pb-4 border-b border-[var(--color-border)]">Community Experience</h2>
              
              <div>
                <label className="block text-sm font-semibold text-[var(--color-text-primary)] mb-4">Would you recommend this area to a friend?</label>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {RECOMMENDATIONS.map(rec => (
                    <label key={rec} className={`flex items-center justify-center p-4 border rounded-[var(--radius-button)] cursor-pointer transition-colors ${data.community.recommend === rec ? 'border-[var(--color-brand-fresh)] bg-[var(--color-surface-soft)] text-[var(--color-brand-deep)]' : 'border-[var(--color-border)] hover:bg-[var(--color-surface-light)] text-[var(--color-text-primary)]'}`}>
                      <input 
                        type="radio" 
                        name="recommend"
                        className="hidden"
              ...
              <div>
                <label className="block text-sm font-semibold text-[var(--color-text-primary)] mb-2">Best Thing About The Area (Optional)</label>
                <textarea 
                  maxLength={250}
                ...
                />
                <div className="text-right text-xs text-[var(--color-text-secondary)] mt-1">{data.community.best_thing.length}/250</div>
              </div>

              <div>
                <label className="block text-sm font-semibold text-[var(--color-text-primary)] mb-2">Biggest Challenge (Optional)</label>
                <textarea 
                  maxLength={250}
                ...
                />
                <div className="text-right text-xs text-[var(--color-text-secondary)] mt-1">{data.community.biggest_challenge.length}/250</div>
              </div>

              <div>
                <label className="block text-sm font-semibold text-[var(--color-text-primary)] mb-2">Suggestion For Improvement (Optional)</label>
                <textarea 
                  maxLength={250}
                ...
                />
                <div className="text-right text-xs text-[var(--color-text-secondary)] mt-1">{data.community.suggestion.length}/250</div>
              </div>
            </div>
          )}
          ...
          {currentStep === 5 && (
            <div className="space-y-8 animate-fade-in">
              <h2 className="text-xl font-bold text-[var(--color-text-primary)] pb-4 border-b border-[var(--color-border)]">Review Your Insights</h2>
              
              <div className="space-y-6">
                <div className="flex justify-between items-start">
                  <div className="grid grid-cols-2 gap-x-12 gap-y-4 w-full">
                    <div className="col-span-2"><h3 className="text-sm font-bold text-[var(--color-text-secondary)] uppercase tracking-wider">Location</h3></div>
                    {Object.entries(data.location).map(([k, v]) => v && (
                      <div key={k}>
                        <span className="block text-sm text-[var(--color-text-secondary)] capitalize">{k.replace('_', ' ')}</span>
                        <span className="block font-medium text-[var(--color-text-primary)]">{v}</span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="flex justify-between items-start pt-4 border-t border-[var(--color-border)]">
                  <div className="grid grid-cols-2 gap-x-12 gap-y-4 w-full">
                    <div className="col-span-2"><h3 className="text-sm font-bold text-[var(--color-text-secondary)] uppercase tracking-wider">Ratings</h3></div>
                    {Object.entries(data.ratings).map(([k, v]) => v > 0 && (
                      <div key={k}>
                        <span className="block text-sm text-[var(--color-text-secondary)] capitalize">{k.replace('_', ' ')}</span>
                        <div className="flex text-[var(--color-brand-medium)]">
                ...
                <div className="flex justify-between items-start pt-4 border-t border-[var(--color-border)]">
                  <div className="grid grid-cols-2 gap-x-12 gap-y-4 w-full">
                    <div className="col-span-2"><h3 className="text-sm font-bold text-[var(--color-text-secondary)] uppercase tracking-wider">Housing Information</h3></div>
                    {Object.entries(data.housing).map(([k, v]) => v && (
                      <div key={k} className="col-span-2">
                        <span className="block text-sm text-[var(--color-text-secondary)] capitalize">{k.replace(/_/g, ' ')}</span>
                        <span className="block font-medium text-[var(--color-text-primary)]">{v}</span>
                      </div>
                    ))}
                  </div>
                  <button type="button" onClick={() => setCurrentStep(3)} className="text-[var(--color-brand-fresh)] text-sm font-semibold hover:underline">Edit</button>
                </div>
                
                <div className="flex justify-between items-start pt-4 border-t border-[var(--color-border)]">
                  <div className="grid grid-cols-2 gap-x-12 gap-y-4 w-full">
                    <div className="col-span-2"><h3 className="text-sm font-bold text-[var(--color-text-secondary)] uppercase tracking-wider">Community Experience</h3></div>
                  ...
                  <button type="button" onClick={() => setCurrentStep(4)} className="text-[var(--color-brand-fresh)] text-sm font-semibold hover:underline">Edit</button>
                </div>
              </div>
