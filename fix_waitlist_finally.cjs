...
// The original file is untouched in memory.
let beforeReturnStr = originalFile.substring(0, originalFile.indexOf('return (\n    <div className="min-h-screen bg-[var(--color-surface-light)] font-sans">'));

beforeReturnStr = beforeReturnStr.replace(/#6FBE45/g, '#008D24');
beforeReturnStr = beforeReturnStr.replace(/#2F8D46/g, '#008D24');
beforeReturnStr = beforeReturnStr.replace(/var\(--color-surface-light\)/g, '#000000');

beforeReturnStr = beforeReturnStr.replace(
...
// Safely remove BannerAnimation
const bannerStart = beforeReturnStr.indexOf('const BannerAnimation');
if (bannerStart !== -1) {
    const bannerEnd = beforeReturnStr.indexOf(');', bannerStart);
    if (bannerEnd !== -1) {
        beforeReturnStr = beforeReturnStr.substring(0, bannerStart) + beforeReturnStr.substring(bannerEnd + 2);
    }
}

// Extract forms from original file
...
let restOfForms = inner.substring(s2Start);

// Colors in restOfForms
restOfForms = restOfForms.replace(/#6FBE45/g, '#008D24');
restOfForms = restOfForms.replace(/#2F8D46/g, '#008D24');
restOfForms = restOfForms.replace(/var\(--color-surface-light\)/g, '#000000');
...
const formArea = newStep1 + restOfForms;
...
const finalRender = newRender.split('FORM_CONTENT_PLACEHOLDER').join(formArea);

fs.writeFileSync('src/pages/WaitlistPage.tsx', beforeReturnStr + finalRender);
console.log("SUCCESS");
