...
if (bannerStart !== -1) {
    ...
    if (bannerEnd !== -1) {
        beforeReturn = beforeReturn.substring(0, bannerStart) + beforeReturn.substring(bannerEnd + 2);
    }
}

// Extract forms from original file (unchanged)
...
const restOfForms = inner.substring(s2Start);
...
fs.writeFileSync('src/pages/WaitlistPage.tsx', beforeReturn + finalRender);
