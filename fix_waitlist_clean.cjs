...
// Also bg-stone-50 inside cards can be tweaked if necessary, but it's fine.
...
// We need to extract the forms accurately
const s2Idx = content.indexOf('{/* STEP 2: PERSONAL DETAILS */}');
const s3Idx = content.indexOf('{/* STEP 3: ROLE SPECIFIC */}');
const s4Idx = content.indexOf('{/* STEP 4: REVIEW */}');
const formEndIdx = content.indexOf('</div>\n                    </motion.div>', s4Idx);

let form2 = content.substring(s2Idx, s3Idx);
let form3 = content.substring(s3Idx, s4Idx);
let form4 = content.substring(s4Idx, formEndIdx);

// The original code has:
// {currentStep === 2 && ( <div className="space-y-6"> ... </div> )}
// We can just keep that exact block. Because we will render them exactly as they are.
// But we want them in a single white card.

const returnIdx = content.indexOf('return (\n    <div className="min-h-screen bg-[var(--color-surface-light)] font-sans">');

const beforeReturn = content.substring(0, returnIdx);

// We will inject a new return statement that re-uses form2, form3, form4 exactly as they were written!
            ...
            <div className="max-w-6xl mx-auto w-full flex-grow flex flex-col">
              ...
              {currentStep === 1 ? (
                ...
                <motion.div
                  ...
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] p-6 md:p-12 w-full max-w-4xl mx-auto"
                >
                  ...
                  {submitError && currentStep === 4 && (
                    <div className="p-5 bg-[#FDEDED] border border-[#F5C2C7] rounded-[18px] text-[#842029] font-medium text-sm mt-6 mb-4">
                      {submitError}
                    </div>
                  )}
...
// Re-inject form2, form3, form4
// We can just concatenate them because they already have {currentStep === X && ...} checks inside them!
const formAreaContent = form2 + form3 + form4;

const completeRender = newRender.replace('FORM_CONTENT_PLACEHOLDER', formAreaContent);
const cleanedBeforeReturn = beforeReturn.replace(/const BannerAnimation = \(\) => \{[\s\S]*?return \([\s\S]*?\}\);/m, '');

fs.writeFileSync('src/pages/WaitlistPage.tsx', cleanedBeforeReturn + completeRender);
